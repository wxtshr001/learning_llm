...
**最终裁决：95/100，通过。第 0002 课正式完成。**

直接索引、具体数值和 Q/K/V layout 三道关键题均已通过，说明 0002 首次提交暴露的主要概念缺口已经补上。学习者指出 rank 检查顺序属于未留意代码逻辑，而非知识概念错误；结合书面题 95 分、主路径 11 项检查通过以及两个错误输入均产生 `ValueError` 的证据，将其降级为非阻塞代码质量问题，不再卡住课程门禁。

按学习者明确要求，本次只保存通过状态，不生成第 0003 课。
...
该顺序仍值得在以后修改生产代码时注意，但它不再被记录为 tensor layout、Python 异常概念或 PyTorch API 的知识缺口，也不要求为此重新提交。
