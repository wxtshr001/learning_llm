...
11. 对完整网络、模块组合和多条 residual 数据依赖，按问题选择最少且有效的图：组件关系复杂时用 UML/结构图，执行顺序复杂时用数据流图，shape 变化复杂时用 shape 表或图，跨 token 关系复杂时用依赖矩阵。不同图不能重复表达同一个结论；动画、聊天和装饰性交互没有新增信息时应删除。
