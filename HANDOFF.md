...
- 第 0003 课首次提交 75/100。代码在 `cuda:0` 上全部通过，dtype 精度与内存计算基本掌握；书面关键题的完整 device/dtype 执行路径未通过，正在进行 0003R 局部补强。
...
遵守 `AGENTS.md`，先拉取 `main`。读取 0003 反馈与 0003R 材料；收到复测后只检查完整执行路径、误差小数位与整数索引语义，未通过关键题时不得生成第 0004 课。
