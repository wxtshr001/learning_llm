...
- 第 0007 课异议复核后 89/100：代码与关键题 1、5、6 通过；只剩关键题 2 未通过。原 Q6.4 的完整 GQA+RoPE+Cache 路径因课程只做一句式预告而撤销。当前 0007R 只补 raw/split/logical/merge tensor 阶段、显式 head axis、concat 与平均；收到 0007R 前不要生成第 0008 课。
...
遵守 `AGENTS.md`，先拉取 `main`。收到 0007R 后复跑独立脚本，并只检查 tensor 阶段、显式 head axis 和 concat/average；不要考完整 GQA+RoPE 系统路径。关键项未通过不得进入 RoPE。
